...
def show_random_patches(dataset, sample_num, rgb_bands=(3, 2, 1)):
    ...
    if (
        len(rgb_bands) != 3
        or any(not isinstance(b, int) for b in rgb_bands)
        or not (1 <= sample_num <= len(static_dataset))
    ):
        del static_dataset
    ...
    sample_indices = np.random.choice(
        len(static_dataset), size=sample_num, replace=False
    )

    fig, axs = plt.subplots(
        nrows=sample_num, ncols=2, figsize=(16, sample_num * 16 / 2), squeeze=False
    )

    for i, sample_index in enumerate(sample_indices):
        # Get RGB data and normalize if necessary
        rgb_data = static_dataset[sample_index][0][list(rgb_bands), :, :].permute(
            1, 2, 0
        )
        if rgb_data.max() > 1:
...
def calculate_global_class_weights(
    src_dir, dataset_name, ignore_index=0, num_classes=14
):
    """
    ...
    weights = (1.0 / ratio) / np.sum(1.0 / ratio)
...
def split_dataset(src_dir, dataset_name, split_ratio, out_dir, seed=0):
    ...
    train_ids = [
        Path(fname).stem.split(".")[0].split("_")[1:] for fname in train_lbl_fnames
    ]
    ...
    val_ids = [
        Path(fname).stem.split(".")[0].split("_")[1:] for fname in val_lbl_fnames
    ]
