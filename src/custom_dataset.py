        ...
        **kwargs,
        ...
        img_fnames = [
            ...
            for (dirpath, dirnames, filenames) in os.walk(
                Path(src_dir) / self.dataset_name
            )
            for f in filenames
            if f.endswith(".tif")
            and ("merged" in f)
            and ("_".join(Path(f).stem.split("_")[1:3]) in flag_ids)
        ]
        img_fnames.sort()
        ...
        lbl_fnames = [
            ...
            if f.endswith(".tif")
            and ("mask" in f)
            and ("_".join(Path(f).stem.split("_")[1:3]).split(".")[0] in flag_ids)
        ]
        lbl_fnames.sort()
        ...
        if self.usage in ["train", "validation"]:
            ...
            for img_fname, lbl_fname in tqdm.tqdm(
                zip(img_fnames, lbl_fnames), total=len(img_fnames)
            ):

                img_chip = load_data(
                    Path(src_dir) / self.dataset_name / img_fname,
                    usage=self.usage,
                    is_label=False,
                ...
                lbl_chip = load_data(
                    Path(src_dir) / self.dataset_name / lbl_fname,
                    usage=self.usage,
                    is_label=True,
                )
        ...
        print(
            f"------ {self.usage} dataset with {len(self.img_chips)} patches created ------"
        )
        ...
        if self.usage in ["train", "validation"]:
            ...
            if self.trans and self.usage == "train":
                ...
                if random.randint(0, 1) and "resize" in self.trans:
                    ...
                    img_chip, lbl_chip = re_scale(
                        img_chip,
                        lbl_chip.astype(np.uint8),
                        scale=scale_factor,
    ...
    def get_metadata_dict(self, meta):
        ...
        for k, v in meta.items():
            if isinstance(v, torch.Tensor):
                ...
                img_meta[k] = [
                    x.item() if isinstance(x, torch.Tensor) else x for x in v
                ]  # get Python number from tensor in list
            else:
                img_meta[k] = v
